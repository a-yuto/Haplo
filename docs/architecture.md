...
P3（型注釈駆動の検査）：`build_shape_env` に **pass3** を足し、型注釈付き関数の引数を
`shape_of_type` で得た宣言 shape に束縛してから本体を `shape_eval` する。これにより
`main` から呼ばれない関数でも本体の固定次元矛盾を検出できる（P2 のボトムアップ推論では
引数が `Unknown` になり見逃していた穴を塞ぐ）。`decompose_arrow` で関数型注釈を
「引数 shape 列＋戻り shape」に分解し、宣言戻り型と本体推論が**両方 `Concrete` で食い違う**
ときだけ `AnnotationMismatch` を報告する（偽陽性ゼロ）。次元変数（`Var`）は注釈から
`DimVal::Var` として保持・伝播するが、単一化・shape 算術は P4 に残す。

なぜ pass3 を「注釈付きのときだけ」走らせるのか：型注釈の無い関数を勝手に検査すると、
引数 shape が決まらず `Unknown` だらけになり益が無いうえ、将来の推論で誤検出の温床になる。
注釈がある関数に限って「宣言を真」として本体を検査する方が、偽陽性ゼロ方針と相性が良い。
